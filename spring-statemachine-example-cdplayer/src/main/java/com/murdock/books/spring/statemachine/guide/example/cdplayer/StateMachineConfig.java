...
public class StateMachineConfig extends EnumStateMachineConfigurerAdapter<State, Event> {
    ...
    public void configure(StateMachineTransitionConfigurer<State, Event> transitions) throws Exception {
        transitions
                .withExternal()
                    .source(State.CLOSED).target(State.OPEN).event(Event.EJECT).and()
                .withExternal()
                    .source(State.OPEN).target(State.CLOSED).event(Event.CLOSE).and()
                .withExternal()
                    .source(State.OPEN).target(State.CLOSED).event(Event.PLAY).and()
                .withExternal()
                    .source(State.PLAYING).target(State.PAUSED).event(Event.PAUSE).and()
                .withExternal()
                    .source(State.PAUSED).target(State.PLAYING).event(Event.RESUME).and()
                .withExternal()
                    .source(State.IDLE).target(State.BUSY).event(Event.PLAY).action(playAction()).guard(playGuard()).and()
                .withExternal()
                    .source(State.BUSY).target(State.IDLE).event(Event.STOP).and()
                .withInternal()
                    .source(State.PLAYING).action(playingAction()).timer(1000).and()
                .withInternal()
                    .source(State.PLAYING).event(Event.BACK).action(backAction()).and()
                .withInternal()
                    .source(State.PLAYING).event(Event.FORWARD).action(forwardAction()).and()
                .withInternal()
                    .source(State.OPEN).event(Event.LOAD).action(loadAction());
    }
